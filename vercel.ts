...
// `next.config.*` at the project root (this Payload template uses
// `next.config.mjs`). Legacy apps generated from the older Vite + Fastify
// template don't — they have a `frontend/` + `backend/` split and own a frozen
// `package.json` from when they were created. Each layout needs different
// Vercel project settings, so this single file branches on the marker file.
