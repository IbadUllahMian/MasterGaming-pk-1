...
export type LoginRequest={email:string;password:string;rememberSession?:boolean};
...
export const authAdapter={
 getSession:async():Promise<AuthSession>=>{const response=await fetch(`${endpoint}/me`,{credentials:'include',cache:'no-store'});if(!response.ok)return null;const body=await response.json() as {user?:PayloadUser};return body.user?toSession(body.user):null;},
 signup:async(request:SignupRequest):Promise<AuthResult>=>{const response=await fetch(endpoint,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({full_name:request.name,username:request.username,email:request.email,password:request.password,mobile_number:request.mobileNumber,free_fire_uid:request.freeFireUid,in_game_name:request.inGameName})});if(!response.ok)return {ok:false,state:'error',message:await messageFor(response)};return {ok:true,state:'created',message:'Your player account has been created. Sign in to continue.'};},
  login:async(request:LoginRequest):Promise<AuthResult>=>{const response=await fetch(`${endpoint}/login`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:request.email,password:request.password})});if(!response.ok)return {ok:false,state:'error',message:await messageFor(response)};const body=await response.json() as {user:PayloadUser};return {ok:true,state:'authenticated',message:'You are signed in.',session:toSession(body.user)};},
 requestPasswordReset:async(request:PasswordResetRequest):Promise<AuthResult>=>{const response=await fetch(`${endpoint}/forgot-password`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:request.email})});return response.ok?{ok:true,state:'created',message:'If an account matches that email, reset instructions have been requested.'}:{ok:false,state:'error',message:await messageFor(response)};},
 logout:async():Promise<void>=>{await fetch(`${endpoint}/logout`,{method:'POST',credentials:'include'});},
