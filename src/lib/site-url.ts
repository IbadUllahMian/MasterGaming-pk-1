// Resolves the deploy URL for the metadata routes (sitemap, robots) and for the
// `metadataBase: new URL(...)` each document's metadata builder sets. Used by
// `app/sitemap.ts`, `app/robots.ts`, and the catch-all route, which hands it to
// `lib/page-metadata.ts`'s `pageMetadata` and `itemMetadata` — so the served
// head and the sitemap advertise the same origin. The root layout
// (`app/(frontend)/layout.tsx`) sets no `metadataBase` of its own.
//
// Precedence:
