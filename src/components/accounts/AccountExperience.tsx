...
export function AuthScreen({ register = false }: { register?: boolean }) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (register) {
      ...
      if (!String(values.identity).trim()) nextErrors.identity = 'Enter your email or username.';
    ...
    const result = register
      ...
      : await authAdapter.login({ identity: String(values.identity), password: String(values.password), rememberSession: values.remember === 'on' } satisfies LoginRequest);
    setLoading(false);
...
function PlayerTournamentsWithErrors() {
  const [registrations, setRegistrations] = useState<{ id: string; status: string; tournament: LiveTournament }[]>([]);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      setError('');
      try {
        const response = await fetch('/cms-api/tournament-registrations?depth=1&limit=100&sort=-createdAt', { credentials: 'include', cache: 'no-store' });
        if (!response.ok) throw new Error('Unable to load your tournament registrations.');
        const payload = await response.json() as { docs: { id: string; status: string; tournament: LiveTournament }[] };
        if (active) setRegistrations(payload.docs);
      } catch (reason) {
        if (active) setError(reason instanceof Error ? reason.message : 'Unable to load your tournament registrations.');
      } finally {
    ...
    return () => { active = false; window.removeEventListener('tournament-registration-created', refresh); };
  }, []);
  return <section className="platform-card mt-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="platform-label">Tournament activity</p><h2 className="mt-2 text-xl font-medium">Joined tournaments</h2></div><Link className="platform-link" href="/tournaments">Find tournaments</Link></div>{loading ? <p className="mt-4 text-sm text-[#a7adb4]">Loading registrations…</p> : error ? <p role="alert" className="mt-4 text-sm text-[#ff9aac]">{error}</p> : registrations.length ? <div className="mt-5 grid gap-3">{registrations.map(registration => <article className="rounded-lg border border-white/8 bg-black/15 p-4" key={registration.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-medium">{registration.tournament.name}</h3><p className="mt-1 text-sm text-[#a7adb4]">{registration.tournament.gameMode} · {registration.tournament.format}</p></div><span className="platform-status upcoming">{registration.status}</span></div><div className="mt-4 grid gap-2 text-sm text-[#a7adb4] sm:grid-cols-2"><p>Upcoming match: {new Date(registration.tournament.schedule).toLocaleString()}</p><p>Registration status: {registration.status}</p></div></article>)}</div> : <p className="mt-4 text-sm text-[#a7adb4]">You have not joined a tournament yet.</p>}</section>;
}

function WalletSummaryWithErrors() {
  const [balance, setBalance] = useState<number>();
  const [error, setError] = useState('');
  useEffect(() => { void (async () => { try { const response = await fetch('/cms-api/wallets?limit=1', { credentials: 'include', cache: 'no-store' }); if (!response.ok) throw new Error('Unable to load your wallet balance.'); const body = await response.json() as { docs: { balance: number }[] }; setBalance(body.docs[0]?.balance || 0); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to load your wallet balance.'); } })(); }, []);
  return <section className="platform-card mt-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="platform-label">Wallet</p><h2 className="mt-2 text-xl font-medium">Available credits</h2></div><Link className="platform-link" href="/credits">View finance activity</Link></div><p className="mt-4 text-3xl font-medium">{balance === undefined && !error ? 'Loading…' : error ? 'Unavailable' : `PKR ${(balance ?? 0).toLocaleString()}`}</p>{error ? <p role="alert" className="mt-2 text-sm text-[#ff9aac]">{error}</p> : <p className="mt-2 text-sm text-[#a7adb4]">Your balance is loaded from the authenticated wallet record.</p>}</section>;
}
...
export function AccountDashboard() {
  ...
  return <main className="platform-shell py-10 sm:py-14">
    ...
    <section className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-4" aria-label="Player account areas">{dashboardCards.map(card => <Link key={card.title} href={card.href} className="platform-card group">
     ...
     {session.user.role === 'Player' && <PlayerTournamentsWithErrors />}
     {session.user.role === 'Player' && <WalletSummaryWithErrors />}
