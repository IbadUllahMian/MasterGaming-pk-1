...
export function TournamentForm({ tournamentId }: { tournamentId?: string }) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); const scheduleValue = String(data.get('schedule')); const scheduleDate = new Date(scheduleValue); const entryFee = Number(data.get('entryFee')); const prizePool = Number(data.get('prizePool')); const maxSlots = Number(data.get('slots')); if (Number.isNaN(scheduleDate.getTime()) || !Number.isFinite(entryFee) || entryFee < 0 || !Number.isFinite(prizePool) || prizePool < 0 || !Number.isSafeInteger(maxSlots) || maxSlots < 2) { setMessage('Enter a valid schedule, non-negative fees and prize pool, and at least two slots.'); return; } setLoading(true); try { const result = await platformCommands.saveTournament({ name: String(data.get('name')).trim(), game: String(data.get('game')), gameMode: String(data.get('mode')), format: String(data.get('matchFormat') || 'Standard').trim(), schedule: scheduleDate.toISOString(), entryFee, prizePool, maxSlots, status: String(data.get('status')), entryType: String(data.get('entryType')), rules: String(data.get('rules')).trim(), participation: String(data.get('participation') || '') }, tournamentId); setMessage(result.message); if (result.ok) window.setTimeout(() => router.push('/admin-panel/tournaments'), 500); } finally { setLoading(false); } };
