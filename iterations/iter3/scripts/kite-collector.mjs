...
function newCollector() {
  return {
    ...
    // Presence of stamp-conditional runtime events the CTA scan never lists:
    // data-kite-item fires item_viewed, data-kite-media fires media_engaged.
    // Item/media VALUES are legitimately dynamic and cannot be catalogued.
    // MEDIA event NAMES are static and authorable: the SDK's wireMedia goes
    // through evName(), which honors an authored data-kite-event — so 2.3
    // manifests record those names, or the catalog would describe an event the
    // runtime never emits. ITEM events are NOT nameable: observeItems captures
    // the literal 'item_viewed' without evName (KEEP IN SYNC with
    // public/kite-analytics.js) — an authored name on an item stamp is inert.
    sawItemStamp: false,
    sawMediaStamp: false,
    mediaEventNames: new Set(), // authored data-kite-event names on media stamps
    sawUnnamedMediaStamp: false, // a media stamp with no authored name (fallback fires)
...
function recordStamp(col, el, sf, relPath, ctx) {
  ...
  // Before the dynamic-stamp early-return below: data-kite-item is exempt from
  // that gate (dynamic by design), and a dynamic stamp elsewhere on the element
  // must not hide that these events fire.
  // The SDK names an item/media event via evName(): the element's authored
  // data-kite-event when present, else the machine fallback. Record any
  // STATIC authored name so the catalog lists the name that actually fires;
  // a dynamic data-kite-event on these stamps degrades to the fallback name
  // (matching the wrong-entry-worse-than-missing rule above).
  const stampEventAttr = readAttr(el, 'data-kite-event', sf);
  const stampEventName =
    stampEventAttr &&
    typeof stampEventAttr.value === 'string' &&
    stampEventAttr.value
      ? stampEventAttr.value
      : null;
  if (readAttr(el, 'data-kite-item', sf) !== null) {
    // ITEM events are NOT nameable (see newCollector): an authored
    // data-kite-event here is inert for the item stream — record only the
    // stamp's presence so item_viewed is listed.
    col.sawItemStamp = true;
  }
  if (readAttr(el, 'data-kite-media', sf) !== null) {
    col.sawMediaStamp = true;
    if (stampEventName) col.mediaEventNames.add(stampEventName);
    else col.sawUnnamedMediaStamp = true;
  }
