...
// 2.2: events additionally lists the stamp-conditional runtime events
// (item_viewed / media_engaged) when their stamps are present. Additive only.
// 2.3: media stamps honor an authored data-kite-event name — the manifest
// records those names in media_event_names (their machine class stays
// media_engaged), events lists each authored name, and the bare
// media_engaged fallback is listed only when an UNNAMED media stamp exists
// to fire it. Item events stay non-nameable (always the literal
// item_viewed). Producer provenance flags (has_item_stamp,
// has_unnamed_media_stamp, has_hooked_conversion_form) record which
// stamp-conditional producers exist so the composer never infers a
// producer from an event name. Additive only.
