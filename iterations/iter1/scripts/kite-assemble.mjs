...
export function assembleManifest(state, srcArg) {
  ...
  // Stamp-conditional runtime events the CTA scan never lists: an element
  // carrying data-kite-item fires item_viewed on exposure, data-kite-media
  // fires media_engaged on play (kite-analytics.js observeItems/wireMedia).
  // ITEM events always land as the literal 'item_viewed' (the runtime skips
  // evName there), so any item stamp lists it. MEDIA event names go through
  // evName(): list each authored name (its machine class rides in
  // media_event_names below), and the 'media_engaged' fallback only when an
  // UNNAMED media stamp exists to fire it — a fallback entry no stamp emits
  // would be a wrong entry. KEEP IN SYNC with public/kite-analytics.js.
  if (state.sawItemStamp && !eventSeen.has('item_viewed')) {
    eventSeen.add('item_viewed');
    events.push('item_viewed');
  }
  const mediaEventNames = [...state.mediaEventNames].sort();
  for (const name of mediaEventNames) {
    if (!eventSeen.has(name)) {
      eventSeen.add(name);
      events.push(name);
    }
  }
  if (state.sawUnnamedMediaStamp && !eventSeen.has('media_engaged')) {
    eventSeen.add('media_engaged');
    events.push('media_engaged');
  }
  ...
  return {
    ...
    primary_conversion_medium: primary
      ...
      : null,
    events,
    // Authored data-kite-event names on media stamps (2.3+): the catalog
    // composer types these to media_engaged rather than deriving a type from
    // the name. (Item events are not nameable — the runtime always emits
    // 'item_viewed'.)
    media_event_names: mediaEventNames,
    // Producer provenance (2.3+): which stamp-conditional runtime producers
    // exist. A closed event NAME in `events` is not proof of its producer —
    // a CTA may be authored with a closed name (e.g. "item_viewed") while no
    // item stamp exists — so the composer reads these flags instead of
    // inferring a producer from the name.
    has_item_stamp: !!state.sawItemStamp,
    has_unnamed_media_stamp: !!state.sawUnnamedMediaStamp,
    has_hooked_conversion_form: !!state.sawHookedConversionForm,
    pages: pageEntries,
    shared,
